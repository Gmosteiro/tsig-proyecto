# Scrip# 3. Mostrar resumen de cambios aplicados
Write-Host "Cambios aplicados para resolver error 429:" -ForegroundColor Cyan
Write-Host "✓ Tamano de tiles aumentado: 256px -> 512px (75% menos requests)" -ForegroundColor Green
Write-Host "✓ Control de flujo optimizado: limites de requests aumentados" -ForegroundColor Green
Write-Host "✓ Timeout WMS aumentado: 10s -> 15s" -ForegroundColor Green
Write-Host "✓ Buffer de tiles aumentado: 0px -> 64px" -ForegroundColor Green
Write-Host "✓ DPI reducido: 96 -> 72 (tiles mas livianos)" -ForegroundColor Green
Write-Host "✓ Compresion DEFLATE habilitada" -ForegroundColor Green
Write-Host "✓ CPU del contenedor aumentado: 2.0 -> 2.5" -ForegroundColor Greenmizacion especifico para error 429 (Too Many Requests)
...
Write-Host "Cambios aplicados para resolver error 429:" -ForegroundColor Cyan
Write-Host "✓ Tamaño de tiles aumentado: 256px → 512px (75% menos requests)" -ForegroundColor Green
Write-Host "✓ Control de flujo optimizado: límites de requests aumentados" -ForegroundColor Green
Write-Host "✓ Timeout WMS aumentado: 10s → 15s" -ForegroundColor Green
Write-Host "✓ Buffer de tiles aumentado: 0px → 64px" -ForegroundColor Green
Write-Host "✓ DPI reducido: 96 → 72 (tiles más livianos)" -ForegroundColor Green
Write-Host "✓ Compresión DEFLATE habilitada" -ForegroundColor Green
Write-Host "✓ CPU del contenedor aumentado: 2.0 → 2.5" -ForegroundColor Green
Write-Host ""
...
# 4. Iniciar servicios en orden optimizado con delays más largos
...
Write-Host "Iniciando GeoServer con configuración optimizada..." -ForegroundColor Gray
docker-compose up -d geoserver
Write-Host "Esperando a que GeoServer esté completamente listo..." -ForegroundColor Gray
...
Start-Sleep -Seconds 5
docker-compose logs --tail=10 geoserver | Select-String -Pattern "429|ERROR|Started"
